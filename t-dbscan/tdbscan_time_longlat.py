...
def duration(a, b):
    return math.fabs(a[2] - b[2])


# 获取满足聚类条件的邻近点
def getNeighbors(i, D, dur, eps):
    neightbors = []
    neightbor_nums = []
    for j in range(i + 1, len(D)):
        if duration(D[i], D[j]) > dur:
            break
        if dist(D[i], D[j]) <= eps:
...
def DBSCAN(D, dur, eps, Minpts):
    ...
    durtmp = dur * 1000
    epstmp = eps / (2 * 6378.137 * 1000)
    for i in range(len(D)):
        neighbors, neightbor_nums = getNeighbors(i, D, durtmp, epstmp)
...
dur = 1800  # 时间间隔，在多少秒范围内
eps = 8000  # 在多少米范围内
minpts = 5  # 超过几人是聚集

C = DBSCAN(dataset, dur, eps, minpts)
